...
VMware Fusion Pro currently only supports MacOS, but if you are on Windows machine, you can use the Windows version, known as VMware Workstation Pro. The functionalities are similar. Alternative tools are also available like Hyper-V manager (Windows), UTM or Parallels. 
...
+ MacOS with Mx series (M1,M2, M3 or M4) chip uses ARM64-based CPU type.
